(() => {
  ...
  function normalizeConversationArchiveLocalPath(value) {
    // 这一段清理当前 Codex text-editor:local tabId、Markdown 链接或 bridge 返回的本机路径，并兼容链接 URI 和编码空格。
    // Clean local paths from current Codex text-editor:local tabIds, Markdown links, or bridge results, including link URIs and encoded spaces.
  ...
  function isConversationArchivePreviewTabPanel(panel) {
    // 这一段只识别当前 Codex text-editor:local 类型的同步归档右侧预览 tab，不兼容旧 tabId。
    // Identify only current Codex text-editor:local archive preview tabs without retaining legacy tabId compatibility.
    ...
    return panel.getAttribute("role") === "tabpanel" &&
      ...
      tabId.startsWith("text-editor:local:") &&
  ...
  function getConversationArchivePreviewDirectory(panel) {
    ...
    const tabId = normalizePath(panel.getAttribute("data-tab-id") || "");
    const localPath = normalizeConversationArchiveLocalPath(tabId.replace(/^text-editor:local:/u, ""));
  ...
  function resolveConversationArchiveThinkingLinkPath(linkTarget, panel) {
    // 这一段把 CodeMirror 文档里的 Markdown 链接转成本机路径，只接受当前预览目录内的 thinking 附件。
    // Convert a Markdown link from the CodeMirror document into a local path, accepting only thinking attachments in the current preview directory.
  ...
  function findConversationArchiveThinkingLinkAtPosition(lineText, lineFrom, position) {
    // 这一段按 Markdown 结构查找点击位置所在的 thinking 链接标签，不依赖“已处理”等显示文案。
    // Find the thinking-link label containing the clicked document position by Markdown structure without relying on display copy.
    const text = String(lineText || "");
    const basePosition = Number(lineFrom);
    const clickedPosition = Number(position);
    if (!Number.isInteger(basePosition) || !Number.isInteger(clickedPosition)) return "";
    const thinkingLinkPattern = /\[([^\]\r\n]+)\]\(<([^>\r\n]*thinking-\d{3,6}-[a-f0-9]{12}\.md)>\)/giu;
    for (const match of text.matchAll(thinkingLinkPattern)) {
      const matchIndex = Number(match.index);
      if (!Number.isInteger(matchIndex)) continue;
      const label = String(match[1] || "");
      const labelFrom = basePosition + matchIndex + 1;
      const labelTo = labelFrom + label.length;
      if (clickedPosition >= labelFrom && clickedPosition <= labelTo) return String(match[2] || "");
    }
    return "";
  }

  function getConversationArchiveThinkingLinkFromEvent(event) {
    // 这一段从真实 CodeMirror 编辑器状态定位点击处的 Markdown 链接，只接管归档预览里的 thinking 附件。
    // Resolve the Markdown link at the click from the real CodeMirror editor state, intercepting only archive-preview thinking attachments.
    const target = event?.target instanceof Element ? event.target : null;
    const panel = target?.closest?.("[role='tabpanel'][data-app-shell-tab-panel-controller='right']");
    if (!isConversationArchivePreviewTabPanel(panel)) return null;
    const content = target?.closest?.("[role='textbox'][contenteditable='true']");
    if (!(content instanceof HTMLElement) || !panel.contains(content)) return null;

    // 这一段用实际点击坐标换算 CodeMirror 文档位置，再读取该位置所在的原始 Markdown 行。
    // Convert the real click coordinates into a CodeMirror document position, then read the raw Markdown line at that position.
    let position;
    let line;
    try {
    ...
    // 这一段只解析点击标签对应的结构化目标，并继续复用同目录 thinking 路径边界检查。
    // Parse only the target for the clicked label and reuse the same-directory thinking path boundary check.
    const linkTarget = findConversationArchiveThinkingLinkAtPosition(line?.text, line?.from, position);
    const localPath = linkTarget ? resolveConversationArchiveThinkingLinkPath(linkTarget, panel) : "";
    return localPath ? { content, localPath } : null;
  }

  function stopConversationArchiveThinkingLinkEvent(event) {
    // 这一段阻止 CodeMirror 继续处理已识别的 thinking 链接点击，后续改用右侧 Preview 打开。
    // Stop CodeMirror from handling an identified thinking-link click before using the right-side Preview opener.
  ...
  runtime.registerSystem("conversation-archive-sidebar", () => {
    ...
    function handleConversationArchivePreviewLinkPointerDown(event) {
      // 这一段只提前接管 Ctrl/Command + 左键，让普通左键继续执行 Codex 原生的光标和选择行为。
      // Intercept only Ctrl/Command plus left-button presses early, preserving Codex's native caret and selection behavior for plain clicks.
    ...
    window.addEventListener("pointerdown", handleConversationArchivePreviewLinkPointerDown, { capture: true, signal: controller.signal });
